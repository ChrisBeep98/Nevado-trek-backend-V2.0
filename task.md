...
### Phase 7: Admin Frontend - Foundation & Auth
145. [x] Initialize Vite Project (`admin-dashboard`) with Tailwind & Framer Motion
146. [x] Setup Project Structure (Contexts, Hooks, Components)
147. [x] Implement Auth Context (Login with Key) & API Client
148. [x] Build Layout Shell (Collapsible Sidebar, Glassmorphism)

### Phase 8: Admin Frontend - Core Features
149. [x] Build Reusable UI Components (Modal, Tabs, Cards)
150. [x] Implement Tours Management (Grid, Create/Edit Modal with new fields)
151. [x] Implement Departures Calendar (FullCalendar integration)
152. [x] Implement Departure Modal (Tabs: Overview, Bookings, Actions)
153. [/] Implement Bookings Management (List, Create Wizard, Detail Modal)
    - [x] List & Details
    - [x] Status & Pax Updates
- Cannot run multiple separate groups for the same tour on the same date
- Customers expecting private tours may end up in public groups
- No way to accommodate different group sizes or special requests on the same date
...
151. [x] Implement Departures Calendar (FullCalendar integration)
152. [x] Implement Departure Modal (Tabs: Overview, Bookings, Actions)
153. [x] Implement Bookings Management (List, Create Wizard, Detail Modal)
    - [x] List & Details
    - [x] Status & Pax Updates
    - [x] Move Booking
    - [ ] Convert Type (Public/Private) UI
154. [x] Implement Dashboard Stats & Polish (Animations, Toasts)

### Phase 9: Frontend Polish & Verification
155. [x] Implement "Convert Type" UI in BookingModal
156. [x] Fix Routing Issue (`/tours` vs `/admin-tours`)
157. [x] Implement Error Boundaries
158. [x] Setup & Write E2E Tests (Playwright)
159. [x] Final Browser Verification & Polish