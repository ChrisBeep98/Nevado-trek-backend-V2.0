...
async function runTransferUpdateDateTest() {
  ...
  try {
    ...
    try {
      ...
      let newDateEventId = null;
      let newDate = null;
      
      // If there are no other events for this tour, create one for 3 days from now
      if (targetTourEvents.length === 0) {
        console.log('   ~ Creating new event for target tour...');
        // This would require creating an event, which isn't directly possible through the API
        // We can try to create a booking which creates an event automatically
        console.log('   ~ Skipping date change test - would require creating new event');
      } else {
        ...
        if (differentDateEvent) {
          // Transfer the new booking to this different date by creating a new booking
          ...
          console.log('   ~ No different date available for transfer test');
        }
      }
